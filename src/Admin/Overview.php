...
class Overview {
	...
	public static function render(): void {
		...
		$cards       = array(
			array( __( 'Points issued', 'infirewards' ), (int) ( $totals['issued'] ?? 0 ), 'dashicons-database-add' ),
			array( __( 'Points redeemed', 'infirewards' ), (int) ( $totals['redeemed'] ?? 0 ), 'dashicons-update' ),
			array( __( 'Outstanding points', 'infirewards' ), $outstanding, 'dashicons-clock' ),
			array( __( 'Rewards redeemed', 'infirewards' ), $reward_count, 'dashicons-tickets-alt' ),
		);
		?>
			...
			<div class="infirewards-overview__grid">
				<?php if ( ! $has_rule || ! $has_reward ) : ?>
					<section class="infirewards-overview__card infirewards-overview__setup">
						<h2><?php esc_html_e( 'Getting started', 'infirewards' ); ?></h2>
						<p><?php esc_html_e( 'Set up points and a reward so customers can start using your program.', 'infirewards' ); ?></p>
						<ol>
							<li><span><strong><?php esc_html_e( 'Create an earning rule', 'infirewards' ); ?></strong><small><?php esc_html_e( 'Choose how many points customers earn per currency unit.', 'infirewards' ); ?></small></span><span class="infirewards-overview__step-action">
							<?php
							if ( $has_rule ) :
								?>
								<span class="infirewards-overview__done"><?php esc_html_e( 'Done', 'infirewards' ); ?></span>
								<?php else : ?>
								<a class="button" href="<?php echo esc_url( admin_url( 'admin.php?page=infirewards-earning-rule' ) ); ?>"><?php esc_html_e( 'Create Rule', 'infirewards' ); ?></a><?php endif; ?></span></li>
							<li><span><strong><?php esc_html_e( 'Create a reward', 'infirewards' ); ?></strong><small><?php esc_html_e( 'Add a discount customers can redeem with points.', 'infirewards' ); ?></small></span><span class="infirewards-overview__step-action">
							<?php
							if ( $has_reward ) :
								?>
								<span class="infirewards-overview__done"><?php esc_html_e( 'Done', 'infirewards' ); ?></span>
								<?php else : ?>
								<a class="button" href="<?php echo esc_url( admin_url( 'admin.php?page=infirewards-rewards' ) ); ?>"><?php esc_html_e( 'Create Reward', 'infirewards' ); ?></a><?php endif; ?></span></li>
							<?php if ( $account_url ) : ?>
								<li><span><strong><?php esc_html_e( 'Check the customer page', 'infirewards' ); ?></strong><small><?php esc_html_e( 'Preview where customers see points and redeem rewards.', 'infirewards' ); ?></small></span><span class="infirewards-overview__step-action"><a class="button" href="<?php echo esc_url( $account_url ); ?>"><?php esc_html_e( 'View page', 'infirewards' ); ?></a></span></li>
							<?php endif; ?>
						</ol>
					</section>
				<?php endif; ?>
				<section class="infirewards-overview__card infirewards-overview__activity">
					<h2><?php esc_html_e( 'Recent activity', 'infirewards' ); ?></h2>
					<?php if ( $activity ) : ?>
						<div class="infirewards-overview__table-wrap"><table class="widefat striped"><thead><tr><th scope="col"><?php esc_html_e( 'Date', 'infirewards' ); ?></th><th scope="col"><?php esc_html_e( 'Customer', 'infirewards' ); ?></th><th scope="col"><?php esc_html_e( 'Activity', 'infirewards' ); ?></th><th scope="col"><?php esc_html_e( 'Points', 'infirewards' ); ?></th></tr></thead><tbody>
						<?php
						foreach ( $activity as $entry ) :
							...
							?>
							<tr><td><?php echo esc_html( mysql2date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $entry['created_at'] ) ); ?></td><td><?php echo esc_html( $user ? $user->display_name : __( 'Deleted customer', 'infirewards' ) ); ?></td><td><?php echo esc_html( $description ); ?></td><td><span class="infirewards-overview__points <?php echo esc_attr( $debit ? 'is-debit' : 'is-credit' ); ?>"><?php echo esc_html( ( 0 === (int) $entry['points'] ? '' : ( $debit ? '−' : '+' ) ) . number_format_i18n( (int) $entry['points'] ) ); ?></span></td></tr>
						<?php endforeach; ?>
						</tbody></table></div>
						<?php
					else :
						?>
						<p class="infirewards-overview__empty"><?php esc_html_e( 'No points activity yet. Transactions will appear here when customers earn or spend points.', 'infirewards' ); ?></p><?php endif; ?>
				</section>
				<section class="infirewards-overview__card infirewards-overview__customers">
					<h2><?php esc_html_e( 'Top customers', 'infirewards' ); ?></h2>
					<?php if ( $customers ) : ?>
						<div class="infirewards-overview__table-wrap"><table class="widefat striped"><thead><tr><th scope="col"><?php esc_html_e( 'Customer', 'infirewards' ); ?></th><th scope="col"><?php esc_html_e( 'Point balance', 'infirewards' ); ?></th></tr></thead><tbody>
						...
						?>
						<p class="infirewards-overview__empty"><?php esc_html_e( 'Customer balances will appear here after points are earned.', 'infirewards' ); ?></p><?php endif; ?>
				</section>
			</div>
		</div>
		<?php
	...
	private static function activity_label( array $entry ): string {
		switch ( $entry['event_type'] ) {
			...
			case 'order_reversal':
				// translators: %d is the WooCommerce order ID.
				return (int) $entry['points'] > 0 ? sprintf( __( 'Reversed points for order #%d', 'infirewards' ), (int) $entry['order_id'] ) : __( 'Order reversal waived', 'infirewards' );
			case 'redemption':
				return __( 'Redeemed a reward', 'infirewards' );
			default:
